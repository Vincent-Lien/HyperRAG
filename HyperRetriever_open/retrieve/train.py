...
if __name__ == "__main__":
    ...
    # --- Configuration ---
    domain = args.domain
    dataset_dir = Path(f"../expr/{domain}/train")
    ...
    if os.path.exists(dataset_save_path):
        ...
        dataset = create_and_save_dataset(json_file_path=json_path, save_path=dataset_save_path)
    ...
    val_dataloader = DataLoader(
        val_dataset, batch_size=batch_size, num_workers=32, shuffle=False
    ...
    log_print(
        ...
        if os.path.exists(dataset_save_path)
        else "Saved dataset not found. Creating a new one..."
    )
    ...
    log_print(
        f"Batch size: {batch_size}, Learning rate: {learning_rate}, Number of epochs: {num_epochs}, Embedding size: {emb_size}"
    )
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs", unit="epoch"):
        ...
        # log_print(f"--- End of Epoch [{epoch+1}/{num_epochs}], Average Train Loss: {avg_train_loss:.4f} ---") # Moved to combined log
        ...
        if avg_val_loss < best_val_loss - min_delta:
            ...
            torch.save(
                {"model_state_dict": model.state_dict(), "pred_in_size": pred_in_size, "emb_size": emb_size},
            ...
            if epochs_no_improve >= patience:
                log_print(
                    f"Early stopping triggered after {epoch+1} epochs due to no improvement in validation loss for {patience} consecutive epochs."
                )
