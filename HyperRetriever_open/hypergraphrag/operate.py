...
def _load_graph_embeddings(domain: str, device) -> Dict[str, torch.Tensor]:
    ...
    """
    embedding_nodes_path = Path(f"../expr/{domain}/graph_embedding_nodes.json")
    embedding_tensor_path = Path(f"../expr/{domain}/graph_embeddings.pt")

    if not embedding_nodes_path.exists() or not embedding_tensor_path.exists():
        logger.error(f"Pre-computed graph embedding files not found in ../expr/{domain}/")
        return {}
...
async def kg_query(
    ...
    if model is None:
        ...
        model_path = f"../expr/{domain}/train/best_retrieval_model.pth"
    ...
    if full_graph is None:
        ...
        graph_path = f"../expr/open_domain/{domain}/graph_chunk_entity_relation.graphml"
