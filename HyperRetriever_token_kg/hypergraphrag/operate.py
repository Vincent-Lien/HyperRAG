...
def _load_graph_embeddings(domain: str, device) -> Dict[str, torch.Tensor]:
    """
    Loads pre-computed graph node and relation embeddings.
    """
    working_dir = Path(f"../kg/wikitopics/{domain}/")
    ...
    if not all([embedding_nodes_path.exists(), embedding_nodes_tensor_path.exists(), 
                embedding_relations_path.exists(), embedding_relations_tensor_path.exists()]):
        logger.error(f"Pre-computed graph embedding files not found in {working_dir}. Please run prepare.py first.")
        return {}

    logger.info("Loading pre-computed graph node and relation embeddings...")
    
    # Load nodes
    with open(embedding_nodes_path, 'r', encoding='utf-8') as f:
        graph_nodes = json.load(f)
    graph_nodes_embeddings = torch.load(embedding_nodes_tensor_path, map_location=device)

    # Load relations
    with open(embedding_relations_path, 'r', encoding='utf-8') as f:
        graph_relations = json.load(f)
    graph_relations_embeddings = torch.load(embedding_relations_tensor_path, map_location=device)
    ...
    logger.info(f"Loaded {len(embedding_map)} graph node and relation embeddings into memory.")
...
async def _score_and_filter_triplets_single_hop(
    ...
    for entity in nodes_to_expand:
        if entity not in full_graph:
            continue
        # Use successors for directed graph traversal
        for next_entity in full_graph.successors(entity):
            # In a DiGraph, get the edge data
            if full_graph.has_edge(entity, next_entity):
                # Assuming one edge between nodes, which is typical for this KG structure
                edge_data = full_graph.get_edge_data(entity, next_entity)
                relation = edge_data.get('relation')
                
                if relation:
                    triplet = (entity, relation, next_entity)
                    # In a directed graph, we don't check for reverse triplets
                    if triplet not in scored_triplets_lookup:
...
async def _build_query_context(
    ...
    logger.info("Attempting to add all triplets in a batch.")
    tentative_triplets_batch = all_scored_triplets
    tentative_entities_batch = {st[0][0] for st in tentative_triplets_batch} | \
                               {st[0][2] for st in tentative_triplets_batch}

    # Build context string for batch
    temp_relations_section_list_data_batch = []
    for (h, r, t), score in tentative_triplets_batch:
        temp_relations_section_list_data_batch.append([h, r, t])

    temp_relations_section_list_batch = [["head", "relation", "tail"]] + temp_relations_section_list_data_batch
    ...
    if len(tokens_batch) <= relation_token_budget:
        ...
        final_relations = {r for h, r, t in [st[0] for st in final_triplets]}
    else:
        # Fallback to iterative filling if batch is too large
        logger.info("Batch too large, falling back to iterative relation filling.")
        current_relation_tokens = 0
        # Sort triplets by score to add the most relevant ones first
        sorted_triplets = sorted(all_scored_triplets, key=lambda item: item[1], reverse=True)

        for (h, r, t), score in sorted_triplets:
            triplet_str = list_of_list_to_csv([["head", "relation", "tail"], [h, r, t]])
            triplet_tokens = len(encode_string_by_tiktoken(triplet_str, model_name=tiktoken_model_name))

            if current_relation_tokens + triplet_tokens <= relation_token_budget:
                final_triplets.append(((h, r, t), score))
                final_relations.add(r)
                final_entities_from_relations.add(h)
                final_entities_from_relations.add(t)
                current_relation_tokens += triplet_tokens
            else:
                break # Stop if budget is exceeded
    
    if not final_triplets:
        logger.warning("No relations could be added to the context within the budget.")
        return "", 0, 0, retrieve_time
    ...
    for (h, r, t), score in final_triplets:
        relations_section_list_data.append([h, r, t])
    
    relations_section_list = [["head", "relation", "tail"]] + relations_section_list_data
    ...
    logger.info(f"Phase 1 complete. Added {len(final_triplets)} triplets. Tokens used: {relations_tokens_used}/{relation_token_budget}. Spillover to entities: {entity_budget_spillover}")
...
async def kg_query(
    ...
    if model is None:
        ...
        model_path = f"../kg/wikitopics/{domain}/train/best_retrieval_model.pth"
    ...
    if full_graph is None:
        ...
        graph_path = f"../kg/wikitopics/{domain}/graph_chunk_entity_relation.graphml"
